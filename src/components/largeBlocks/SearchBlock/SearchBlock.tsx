...
const SearchBlock = ({
  ...
  const [searchIngredient, setSearchIngredient] = useState<ingredientItem>(
    activeIngredients[0],
  );

  const handleActiveIngredients = (ingredient: ingredientItem) => {
    setActiveIngredients((prev) => {
      const isActive = prev.some((elem) => elem.id === ingredient.id);

      return isActive
        ? prev.filter((elem) => elem.id !== ingredient.id)
        : [...prev, ingredient];
    });
  };
      ...
      <div className="flex gap-20 justify-center items-center">
        <div>
          ...
          >
            {ingredients.map((ingredient) => (
              <button
                ...
                key={ingredient.id}
                className={`
            ${activeIngredients.find((elem) => elem.id === ingredient.id) ? "bg-orange" : "bg-lightPink"}
          flex gap-x-2 items-center font-spectral lowercase text-[1.3rem] rounded-4xl p-2 transition duration-300 hover:bg-orange bg-lightPink  text-black cursor-pointer shadow-[0_4px_10px_rgba(0,0,0,0.25)]  px-4`}
                onClick={() => handleActiveIngredients(ingredient)}
              >
